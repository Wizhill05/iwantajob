    ...
    <header className="sticky top-0 z-20 bg-[#131313]/80 backdrop-blur-md border-b border-[#262626] px-4 lg:px-8 py-3 flex items-center justify-between min-h-[57px]">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        {runningCount > 0 ? (
          <Link
            href="/logs"
            className="flex items-center gap-2 px-2.5 py-1 rounded-full bg-[#3ecf8e]/10 border border-[#3ecf8e]/30 text-[#3ecf8e] text-xs font-mono transition-all hover:bg-[#3ecf8e]/20"
          >
            <Activity className="w-3.5 h-3.5 animate-spin" />
            <span className="hidden sm:inline">{runningCount} active task{runningCount > 1 ? 's' : ''}</span>
            <span className="sm:hidden">{runningCount} active</span>
          </Link>
        ) : (
          <div className="hidden md:flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[#181818] border border-[#262626] text-[11px] font-mono text-[#6b7280]">
        ...
        <div
          className="flex items-center gap-2 px-2.5 py-1 rounded-full bg-[#181818] border border-[#262626] text-xs font-mono"
          title={
            isOnline === true
        ...
        >
          <span className="relative flex h-2 w-2">
            {isOnline === true ? (
              ...
              <span className="relative inline-flex rounded-full h-2 w-2 bg-rose-500 shadow-[0_0_6px_rgba(244,63,94,0.6)]"></span>
            ) : (
